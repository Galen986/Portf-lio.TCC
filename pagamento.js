...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Função principal de cálculo/atualização do resumo
  function atualizarResumoPedido() {
    ...
    if (cupomAplicado) {
      ...
      // Aplica frete grátis se houver
      if (cupomAplicado.freteGratis) {
        freteGratisDiv.style.display = "block";
      } else {
        freteGratisDiv.style.display = "none";
      }
    } else {
      // Não exibe desconto/frete grátis se não houver cupom
      descontoAplicadoDiv.style.display = "none";
      freteGratisDiv.style.display = "none";
    }
    ...
    // Atualiza as opções de parcelamento, se existirem
  ...
  // Aplica cupom quando clicar no botão
  if (aplicarCupomBtn) {
  ...
  // Chama ao iniciar para montar o resumo
  atualizarResumoPedido();

  // Se quiser, chame atualizarResumoPedido() sempre que adicionar/remover produtos no carrinho!
});
